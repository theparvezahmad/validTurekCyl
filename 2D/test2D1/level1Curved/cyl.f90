program cyl
   ...
   integer:: i, j, a, a1, t_, ia, ja, solnumber, cnt
         ...
         if (isn(i, j) == 0) then
            do a = 0, q - 1
               ...
               if (isn(ia, ja) == 1) then !Cylinder
                  ...
                  cnt = cnt + 1
                  Delta(cnt) = dsqrt((i - ii)*(i - ii) + (j - jj)*(j - jj))/dsqrt(dble(i - ia)*(i - ia) + dble(j - ja)*(j - ja))
               ...
               do a = 0, q - 1
                  ...
                  if (isn(ia, ja) .eq. 1) then !structure
                     ...
                     ft(kb(a), ia, ja) = ft(a, i, j) - chi*(ft(a, i, j) - feq) + &
                                   wt(a)*rho(i, j)*3.0d0*(ex(a)*(ubfx - ux(i, j) - 2.0d0*uwx) + ey(a)*(ubfy - uy(i, j) - 2.0d0*uwy))

                     ! tmp1 = -ux(i, j)*ex(a) - uy(i, j)*ey(a)
                     ! tmp2 = ux(i, j)*ux(i, j) + uy(i, j)*uy(i, j)
                     ! feq = wt(a)*rho(i, j)*(1.0d0 + 3.0d0*tmp1 + 4.5d0*tmp1*tmp1 - 1.5d0*tmp2)
                     ! ft(a, ia, ja) = ft(kb(a), i, j) - chi*(ft(kb(a), i, j) - feq) + &
                     !              wt(a)*rho(i, j)*3.0d0*(-ex(a)*(ubfx - ux(i, j) - 2.0d0*uwx) - ey(a)*(ubfy - uy(i, j) - 2.0d0*uwy))

                     fx(2) = fx(2) + ex(a)*(ft(a, ia, ja) + ft(kb(a), i, j))
                     fy(2) = fy(2) + ey(a)*(ft(a, ia, ja) + ft(kb(a), i, j))
...
!----------------------------------------------------------------------
      if (t_ .le. time_ .and. mod(t_, (time_/(noOfSnaps - 1))) .eq. 0) then
         ...
         close (12)
         write (*, '(a,I3,a,I10)') "snap", solnumber, " recorded at LBM time ", t_
      end if
!----------------------------------------------------------------------
